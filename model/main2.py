...
def lossbatchtest(model, x_catgs, x_conts, Gs, inputss, data_id_alls, vocab_to_ints, step, label_stab, label_repu, label_act,
    ...
    return total_loss, loss1, loss2, loss3, loss4, loss5, loss6,\
           predict_stab,label_stab,predict_repu,label_repu,\
...
def test(model, x_catgs_valids, x_conts_valids, Gs_valids, inputss_valids, data_id_alls_valids,
    ...
    # regression metrics
    loss3_mae,loss4_mae,loss5_mae,loss6_mae = 0,0,0,0
    ...
    with torch.no_grad():
        for step in range((data_guild_valids['guild_id'].shape[0] - 1) // BATCHSIZE + 1): # 为哈在这里没帮其切分呢。
           ...
           loss3_mae,loss4_mae,loss5_mae,loss6_mae = lossbatchtest(model, x_catgs_valids, x_conts_valids, Gs_valids, inputss_valids, data_id_alls_valids,
            ...
            # metric
            # predict_stabs = torch.cat(predict_stabs,predict_stab)
            # label_stabs = torch.cat(label_stabs,label_stab)
            # predict_repus = torch.cat(predict_repus,predict_repu)
            # label_repus = torch.cat(label_repus,label_repu)
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adam(itertools.chain(model.parameters(),awl.parameters()), lr=1e-5) # weight_decay=1e-5
    # 读取数据
    ...
    store_data = {}
    for i in range(10): # 这里才是真正的8个epoch
        EPOCHS = 2
        # metric
        ...
        for ds_end in ds_ends:
            try:
                if i ==0:
                    ...
                    label_stab, label_repu, label_act, label_commu, label_reso, label_ability = store_data[ds_end] # 14天的都全了

                else:
                    data_guild, x_catgs, x_conts, wide_dim, dense_dim, embed_dim, \
                    Gs, inputss, data_id_alls, vocab_to_ints, \
                    label_stab, label_repu, label_act, label_commu, label_reso, label_ability = store_data[ds_end]
                main()
